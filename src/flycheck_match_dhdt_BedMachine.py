...
dz_smoothed_10 = gaussian_filter(dz, 10)
dz_smoothed_15 = gaussian_filter(dz, 15)

dz_filled = dz.copy()
dz_filled_smoothed_10 = dz_smoothed_10.copy()
dz_filled_smoothed_15 = dz_smoothed_15.copy()

dz_filled[~np.isfinite(dz_filled)] = bedmachine_grid_m[:, :, 2][~np.isfinite(dz_filled)]

dz_filled_smoothed_10[~np.isfinite(dz_filled_smoothed_10)] = bedmachine_grid_m[:, :, 2][
    ~np.isfinite(dz_filled_smoothed_10)
]

dz_filled_smoothed_15[~np.isfinite(dz_filled_smoothed_15)] = bedmachine_grid_m[:, :, 2][
    ~np.isfinite(dz_filled_smoothed_15)
]
...
if wo:
    ...
    with rasterio.open(
        ...
        dst.write(dz_filled_smoothed_10, 1)
